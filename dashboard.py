...
st.header("Selecione o mês e o ano para visualizar as despesas:")
...
st.header(f'Despesas do mês de {mes} de {ano} por categoria:')
st.dataframe(sumCategoria,column_config=config,hide_index=True)
st.header(f'Todas as despesas do mês de {mes} de {ano}:')
...
with tab1:
    ...
    st.line_chart(dfChart, x='month', y='Valor', x_label='Mês', y_label='Valor (R$)', use_container_width=True)
